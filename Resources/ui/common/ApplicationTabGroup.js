function ApplicationTabGroup(GA_Master) {


	Ti.include('/jslib/fnc_cloud.js');

	// definer Google Analytics tracker
	var tracker_master;
    var winQRAccount = undefined;
	...
	var loadingTabs = function(tabgroupWin,startup){
		...
		// alert('Login with Clouduser:'+Ti.App.Properties.getString('cloud_useremail','No User Set'))
		...
		Cloud.Users.login({
			...
			if (e.success) {
				...
				Ti.App.Properties.setString('QRAppContentTitle',(cloudContentTitle == undefined)?L('_MCMSCONTENTTITLE','mCMS Content'):cloudContentTitle);
	...
	if (defaultTabs){	
		...
		if (localTabs){
			...
			// var defaultJSON is configered in conf/default.json
			...
			if (((Ti.App.Properties.getString('loading_method','TAB')=='TAB') && (Ti.App.Properties.getBool('auto_login',false)))
				...
				// start to load Customized Tabs		
				loadingTabs(self_win,true);					
			} else {
				// start to load Default Tabs	
				loadingDefaultTabs(self_win,true);	    	
			};	
		};
	};
