...
function dbInitialize(){
	...
	db_QR.execute('CREATE TABLE IF NOT EXISTS QRHISTORY (QRFILENAME TEXT, URL TEXT , DESC TEXT);');
	db_QR.execute('CREATE INDEX IF NOT EXISTS QRFILENAME ON QRHISTORY(QRFILENAME ASC);');
...
// get QR Hist Descriptions
function buildQRDesc(QR_filename,QR_link,QR_desc){
	var db_QR = Titanium.Database.open(db_QR_name);	
	if (QR_desc = undefined) {
		QR_desc = ''; 
	}
	var rows = db_QR.execute('INSERT INTO QRHISTORY VALUES(?,?,?);',QR_filename,QR_link,QR_desc);
	db_QR.close();	
}
...
function show_links(url_Link){
	...
	var bb_back = Titanium.UI.createButton({
		image:'image/toolbar_leftarrow.png'
	});
	...
	var bb_forward = Titanium.UI.createButton({
		image:'image/toolbar_rightarrow.png'
	});
	...
	var bb_reload = Titanium.UI.createButton({
		image:'image/toolbar_refresh.png'
	});
